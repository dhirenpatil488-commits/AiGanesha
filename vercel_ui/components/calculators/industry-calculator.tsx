  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-border/30">
      ...
      <Input
        ...
        value={value}
        onChange={onChange}
        className={`${type === "text" ? "w-48" : "w-32"} bg-secondary/50 border-border text-right`}
      />
    </div>
  );
}
...
function SectionHeader({ title }: { title: string }) {
  return (
    <h3 className="text-sm font-semibold text-primary border-b border-primary/30 pb-2 mt-6 mb-4">
      {title}
    </h3>
  );
}
...
export default function IndustryCalculator({ onBack }: IndustryCalculatorProps) {
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-8">
      <div className={`max-w-${step === "results" ? "6xl" : "3xl"} mx-auto transition-all duration-300`}>
        {/* Back Button */}
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-foreground mb-2">GHG Protocol Hub</h1>
          {step === "hub" && !isEditingFacility && (
            <p className="text-muted-foreground">ISO 14064 Facility-Based Architecture</p>
          )}
        </div>
            ...
            {step === "org" && (
              <div className="space-y-6">
                ...
                <div className="space-y-3 mt-6">
                  <Label className="text-foreground font-medium">Primary Industry Sector</Label>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl font-semibold text-foreground">Facilities Network</h2>
                  </div>
                  <Button variant="outline" onClick={addFacility} className="border-primary text-primary hover:bg-primary/10">
                    <Plus className="w-4 h-4 mr-2" /> Add Physical Location
                  </Button>
                </div>
                  ...
                  {data.facilities.map((fac) => (
                    ...
                    >
                      ...
                      <Button
                        ...
                        className={fac.isComplete ? "bg-emerald-600 hover:bg-emerald-700" : "bg-primary hover:bg-primary/90"}
                      >
                        {fac.isComplete ? (
                          <>
                            <CheckCircle className="w-4 h-4 mr-2" /> Data Ready
                          </>
                        ) : (
                          "Enter Data"
                        )}
                ...
                <div className="flex justify-between pt-6 border-t border-border">
                  ...
                  >
                    Proceed to Org-Level Emissions <ChevronRight className="w-4 h-4 ml-2" />
                  </Button>
                </div>
              </div>
            )}
            ...
            {step === "org_scope3" && (
              <div className="space-y-6">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-xl font-semibold text-foreground">Organization-Level Upstream/Downstream</h2>
                </div>
                ...
                <div className="flex justify-between pt-6 border-t border-border">
                  <Button variant="outline" onClick={() => setStep("hub")}>
                    <ChevronLeft className="w-4 h-4 mr-2" /> Back to Facilities
                  </Button>
                  <Button onClick={calculateResults} className="bg-primary hover:bg-primary/90">
                    Calculate ESG Report <Activity className="w-4 h-4 ml-2" />
                  </Button>
                </div>
              </div>
            )}
